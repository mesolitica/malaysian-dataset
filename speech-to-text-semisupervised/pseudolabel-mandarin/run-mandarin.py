...
class Train(Dataset):
    def __init__(self):
        mandarin = glob('data_aishell/wav/train/*/*.wav')
        mandarin.extend(glob('split-train_M/*.mp3'))
        mandarin.extend(glob('split-Train_Ali/*.mp3'))
        mandarin.extend(glob('split-youtube-mandarin/*.mp3'))
        mandarin.extend(glob('train/*/*.wav'))

        self.files = sorted(mandarin)
...
class ModelArguments:
    ...
    folder_output: str = 'output-mandarin'
    batch_size: int = 8
...
def main():
    ...
    for step, batch in enumerate(batches):
        ...
        last_logits_zh = torch.stack(output_zh.scores, dim=1)
        m1 = (last_logits_zh.argmax(dim=-1) < processor.tokenizer.vocab_size)
        m2 = (last_logits_zh.argmax(dim=-1) != processor.tokenizer.pad_token_id)
        mask = m1 & m2
        last_logits_zh[~mask] = 0.0
        prob_score_zh = last_logits_zh.max(dim=-1).values.mean(dim=1)
        prob_score_zh = prob_score_zh.detach().cpu().type(torch.float32).numpy().tolist()

        data = []
        for k in range(len(batch)):
            data.append(
                {
                    'predict_zh': output_zh.sequences[k].tolist(),
                    'score_zh': prob_score_zh[k],
