...
class Train(Dataset):
    def __init__(self):
        self.files = sorted(glob('split-indonesian/*.mp3'))
...
class ModelArguments:
    ...
    folder_output: str = 'output-indonesian-v2'
    batch_size: int = 8
...
def main():
    ...
    for step, batch in enumerate(batches):
        ...
        last_logits_id = torch.stack(output_id.scores, dim=1)
        m1 = (last_logits_id.argmax(dim=-1) < processor.tokenizer.vocab_size)
        m2 = (last_logits_id.argmax(dim=-1) != processor.tokenizer.pad_token_id)
        mask = m1 & m2
        last_logits_id[~mask] = 0.0
        prob_score_id = last_logits_id.max(dim=-1).values.mean(dim=1)
        prob_score_id = prob_score_id.detach().cpu().type(torch.float32).numpy().tolist()
        ...
        for k in range(len(batch)):
            data.append(
                {
                    'predict_id': output_id.sequences[k].tolist(),
                    'score_id': prob_score_id[k],
